//
//  MyDoctorScheduleControllerViewController.swift
//  XRAI
//
...
class MyDoctorScheduleControllerViewController: UIViewController {
...
extension MyDoctorScheduleControllerViewController: UITableViewDelegate, UITableViewDataSource{
    ...
    func initializeEvents(){
        ...
        ref.observe(.childAdded) { (snapchot) in
            ...
            if let dictionnaire = snapchot.value as? NSDictionary{
                ...
                print(dictionnaire)
                if((dictionnaire["isBooked"] as? Bool ?? false) && dictionnaire["doctorId"] as? String ?? "error" == Auth.auth().currentUser?.uid ){
                    
                    let schedule = Schedule(id: snapchot.key, date: dictionnaire["date"] as? String ?? "2021-04-26T14:00:00.000+0200", doctorId: dictionnaire["doctorId"] as? String ?? "zEeDO8mGPoPEc30RCENYZsEvSQ23", isBooked: true, bookedBy: dictionnaire["bookedBy"] as? String ?? "zEeDO8mGPoPEc30RCENYZsEvSQ23")
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "rdvPatientCell", for: indexPath) as! RdvPatienCell
        ...
        let ref = Database.database(url: "https://xrai-bb84c-default-rtdb.europe-west1.firebasedatabase.app/").reference().child("users").child(activities[indexPath.section].bookedBy!)
        ref.observeSingleEvent(of: DataEventType.value, with: { (snapshot) in
            ...
            cell.mailLbl.text = value?["mail"] as? String ?? "8 rue amédée Simon Villeneuve le roi 94290"
        })
